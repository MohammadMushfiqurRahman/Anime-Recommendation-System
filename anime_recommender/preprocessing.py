...
def clean_text(text):
    """Clean text data by removing special chars and converting to lowercase"""
    if pd.isna(text):
        return ""
...
def preprocess_anime_data():
    ...
    df = pd.read_csv(
        "65k_anime_data.csv", encoding="utf-8", encoding_errors="ignore"
    )
    ...
    df_processed = df_processed[
        df_processed["combined_features"].str.strip() != ""
    ]
